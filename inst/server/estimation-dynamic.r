...
server.dynamic.d2003.computeclusters <- function() {
  ...
  load("L:\\Tibo\\qtrdata\\tmp-2003-dynamic.dat")
  sdata = sdata[move==FALSE]
  sim=list(sdata=sdata,jdata=jdata)
  rm(sdata,jdata)
  
  set.seed(954938257)
  rs      = rkiv0.start("m4-mixt-2003data-groups")
  rs$info = "dynamic 2003 data cluster outcome for mixture"
  rs$location = "serverdata"
...
server.dynamic.d2003.clustering.stats <- function() {
  ...
  # get the groups
  grps = rkiv0.load("m4-mixt-2003data-groups")
  sim   = grouping.append(sim,grps$best_cluster)  
  
  rs      = rkiv0.start("m4-stats-d2003")
  rs$info = "simple statistics on 2003 dynamic data"
  ...
  rkiv0.put(rs,list(mstats=mstats,cstats=cstats,gstats = gstats))
}
...
server.static.rho.analysis <- function(){
  ...
  archive.put(rhos_grid=rr,m="slices values for rho1, rho4 and rho32",file = arch_dyn)
}
...
server.static.mini.estimate.main <- function(){
  ...
  archive.put(mstats=mstats,m="mean and sd by cluster for movers",file = arch_dyn)
  archive.put(cstats=cstats,m="mean and sd by cluster for stayers",file = arch_dyn)
  ...
  archive.put(mini_model=model_mini,m="main mini model estimation dynamic",file = arch_dyn)
  archive.put(mini_model_vdec=vdec_minimodel,m="main mini model variance decomposition dynamic",file=arch_dyn)
  ...
  archive.put(reg_analysis=reg,m="endogeneity analysis on data simulated from mini model",file = arch_dyn)
...
server.dynamic.mini.estimate.bootstrap <-function() {
  ...
  arch_dyn = "res-2003-dynamic.dat"
  # res = rkiv.res("dyn-mixt-long") + 
  #        rk_dep("data-dyn-long") + rk_loc("server") + rk_start()
  load("L:\\Tibo\\qtrdata\\tmp-2003-dynamic.dat")
  sim=list(sdata=sdata,jdata=jdata)
  rm(sdata,jdata)
  ...
  archive.put(mini_model_prof=model0,m="main mini prof, 2003 data",file = arch_dyn)
  archive.put(mini_model_prof_bis=res_rhos,m="main mini prof, 2003 data rhos",file = arch_dyn)
  archive.put(mini_model_prof_vdec_bis=vdec2,m="main mini prof, 2003 data vdec",file = arch_dyn)
  ...
  archive.put(mini_model_bs=rr_mini,m="main mini boostrapped, 2003 data",file = arch_dyn)
  archive.put(mini_model_bs_vdec=rrr,m="variance decomposition for main mini boostrapped, 2003",file = arch_dyn)
...
server.dynamic.mini.estimate.explore_rho <- function(){
  ...
  archive.put(mini_model_vedc_rhogrid=rrr,m="mini model estimated with different rhos",file = arch_dyn)
}
...
server.dynamic.mixture.d2003.estimate <- function() {
  ...
  # get main estimate
  res_main  = rkiv0.load("m4-mixt-d2003-main")
  ...
  # ---- all best 10 liks ------ #
  res_mixt = rkiv0.load("m4-mixt-d2003-main")
...
server.dynamic.mixture.d2003.fit <- function() {
  ...
  rkiv0.put(rs,list(dd_s=dd_s,dd_m=dd_m,rr_s=rr_s,rr_m=rr_m,dd_s_g=dd_s_g,dd_m_g=dd_m_g))
...
server.dynamic.mixture.estimate.robust.different.rho <- function() {
  ...
  res_mixt_rho = rkiv0.load("m4-mixt-d2003-rho-check")
...
server.dynamic.mixture.d2003.boostrap <- function(){
  ...
  rkiv0.put(rs,list(vdecs=rr,mixt_all=rr_mixt_all))
...
server.dynamic.mixture.estimate.2003data.localmax <- function() {
  ...
  # get best estimates
  res_mixt = rkiv0.load("m4-mixt-2003data-main-para-bis")
  ...
  res = rkiv0.load("m4-mixt-2003data-main-para")
  ctrl      = em.control(est_rho=c(FALSE,TRUE,FALSE,FALSE,TRUE,FALSE),
                         nplot=500,tol=1e-7,dprior=1.001,fixb=TRUE,
                         sd_floor=1e-7,posterior_reg=1e-8,ncat=1,
                         est_rep=50,est_nbest=10,sdata_subsample=0.1)
  ...
  res = rkiv0.load("m4-mixt-2003data-main")
  ctrl      = em.control(est_rho=c(FALSE,TRUE,FALSE,FALSE,TRUE,FALSE),
                         nplot=50,tol=1e-7,dprior=1.001,fixb=TRUE,
                         sd_floor=1e-7,posterior_reg=1e-8,ncat=1,
                         est_rep=50,est_nbest=10,sdata_subsample=0.1)
