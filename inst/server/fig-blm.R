...
suppressMessages(suppressWarnings({require(plyr);require(blm);require(grid)}))
require(brew)
...
table.static.clusters <- function() {
  ...
  m2stats = rkiv0.load("m2-stats-d2003")
  ...
  m4stats = rkiv0.load("m4-stats-d2003")
...
table.dynamic.clusters <- function() {
  # create table for static
  m2stats = rkiv0.load("m4-stats-d2003")
  gstats = m2stats$gstats
...
wages.movers <- function() {
  mstats = rkiv0.load("m2-stats-d2003")$mstats
...
fig.static.mixt.means <- function() {
  ...
  cstats        = rkiv0.load("m2-stats-d2003")$cstats
  ...
  wplot(res_main$model$A1[,I]) + xlab("firm class k") +theme_bw() +
  ...
  ggsave("~/Dropbox/paper-blm1/figures/build/fig-mixt2-d2003-wage.pdf",gp,width = 6.5,height = 5)
  ...
  ggsave("~/Dropbox/paper-blm1/figures/build/fig-mixt2-d2003-wage-uc.pdf",gp,width = 6.5,height = 5)
  ...
  ggsave("~/Dropbox/paper-blm1/figures/build/fig-mixt2-d2003-pk.pdf",gp,width = 6.5,height = 5)
}
...
table.static.mixt.vdec <- function() {
  ...
  rr_mean_effects = rkiv0.load("m2-mixt-d2003-meffects-leg2")
...
table.dynamic.mixt.vdec <- function() {
  ...
  # ===== EXTRACT REALLOCATION RESULTS ====== #  
  rr_mean_effects = rkiv0.load("m4-mixt-d2003-meffects-leg2")
  res_means         = df2list(rr_mean_effects$diff,"main")
  res_means_sd      = df2list(rr_mean_effects$diff,"sd")
  res_means_m1      = df2list(rr_mean_effects$diff,"m1")
  res_means_q0      = df2list(rr_mean_effects$diff,"q0")
...
fig.static.mixt.means.residuals <- function() {
  res_main      = rkiv0.load("m2-mixt-d2003-main-residuals")
  cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
fig.static.mixt.means.withx <- function() {
  res_wx = rkiv0.load("m2-mixt-d2003-main-withx")
...
fig.static.mixt.splits <- function() {
  res_split = rkiv0.load("m2-mixt-y2003-splits")
...
fig.static.mixture.fit <- function() {
  m2.fit = rkiv0.load("m2-mixt-d2003-main-fit")
...
fig.akm <- function() {
  res_akm  = rkiv0.load("akm-fe-bc-d2003")
...
fig.dynamic.mixt.means <- function() {
  ...
  cstats        = rkiv0.load("m4-stats-d2003")$cstats
  cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
fig.dynamic.mixt.splits <- function() {
  res_split  = rkiv0.load("m4-mixt-d2003-splits")
...
fig.dynamic.mixture.fit <- function() {
  m2.fit = rkiv0.load("m4-mixt-d2003-main-fit")
...
fig.dyn.rho.fit <-function() {
  ...
  res_main_rho   = rkiv0.load("m4-mixt-d2003-rho-check")
...
table.statedependence <- function() {
  ...
  rr = rkiv0.load("m4-cf-state-dependence")
  r1 =as.list(rr$main)
...
fig.shimersmith.model <- function() {
  r = rkiv0.load("m2-shimersmith-mc") 
  # make figure
...
fig.shimersmith.wages <- function() {
  ...
  res_ss = rkiv0.load("m2-shimersmith-mc")
...
fig.shimersmith.narrow <- function() {
  rr = rkiv0.load("m2-shimersmith-narrow")
...
plot.static.proba.gibbs <- function() {
  m2proba = rkiv0.load("m2-proba-gibbs-d2003-res")
...
figure.hybrid <- function() {
  ...
  rr = data.table(rkiv0.load("m2-trace-d2003-withinRe"))
  rr[, akm_share := 0.32]  
...
tab.akm_discretization <- function() {
  rr = rkiv0.load("m2-akm-effect-of-discretization")
...
tab.dirty_iteration <- function() {
  rr = data.table(rkiv0.load("m2-mixt-d2003-main-reclassify-allresults"))
