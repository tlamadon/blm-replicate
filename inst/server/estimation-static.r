...
server.static.d2003.clustering <- function() {
  ...
  set.seed(65542134)
  rs    = rkiv0.start("m2-mixt-y2003-groups",info="static 2003 data cluster outcome for mixture",location = "serverdata")
  ...
  rr = data.frame()
  for (i in 1:20) {
    grps.tmp  = grouping.classify(ms,ksupp = 10,stop = FALSE,nstart = 10,verbose =10,iter.max = 200)
    rr = rbind(rr,grps.tmp$summary)
  }
  rr = data.table(rr)
}
...
server.static.d2003.clustering.stats <- function() {
  ...
  rs      = rkiv0.start("m2-stats-d2003")
  rs$info = "simple statistics on 2003 static data"
  ...
  my = sdata[,list(m=mean(y1),.N),f1][,sum(N*m)/sum(N)]
  v1 = sdata[,list(m=mean(y1),.N),f1][,sum(N*(m-my)^2)/sum(N)]
  my = sdata[,list(m=mean(y1),.N),j1][,sum(N*m)/sum(N)]
  v2 = sdata[,list(m=mean(y1),.N),j1][,sum(N*(m-my)^2)/sum(N)]
  v2/v1
...
server.static.d2003.clustering.robust <- function() {
  load("L:\\Tibo\\qtrdata\\tmp-2003-static.dat")
  sdata[,x:=1][,y1_bu:=y1]
  sim = list(jdata=jdata,sdata=sdata)
  rm(sdata,jdata)
  ...
  rs    = rkiv0.start("m2-mixt-y2003-groups",info="static 2003 data cluster outcome for mixture",location = "serverdata")
  ms    = grouping.getMeasures(sim,"ecdf",Nw=20,y_var = "y1")
  grps  = grouping.classify(ms,ksupp = 10,stop = FALSE,nstart = 10000,verbose =10,iter.max = 200)
  sim   = grouping.append(sim,grps$best_cluster)
  rkiv0.put(rs,grps)
...
server.static.mini.estimate.main <- function(){
  ...
  archive.put(mstats=mstats,m="mean and sd by cluster for movers",file = arch_static)
  archive.put(cstats=cstats,m="mean and sd by cluster for stayers",file = arch_static)
  
  mini_model = m2.mini.estimate(jdata,sdata,norm = 1,method="prof")
  m2.mini.plotw(mini_model)
  
  sdata.sim  = m2.mini.impute.stayers(mini_model,sdata)
  vdec_minimodel = lin.proja(sdata.sim,"y1_imp","k_imp","j1")
  
  archive.put(mini_model_prof=mini_model,m="main mini model estimation,profiling B2",file = arch_static)
  archive.put(mini_model_prof_vdec=vdec_minimodel,m="main mini model variance decomposition,profiling B2",file=arch_static)

  # re-simulate once - recluster
  clus   = cluster.firms.data(sdata.sim,fcol = "f1",ycol = "y1_imp",ncluster = 10)
  sdata.sim = cluster.append.data(sdata.sim,clus,fcol="f1",jcol="j1",type="distr")
  
  sdata.sim[,k_hat:=mean(y1_imp),list(j1)]
}
...
server.static.mini.estimate.bootstrap <-function() {
  ...
  model0 = archive.get("mini_model_prof",file=arch_static) # create a dependency
